...
export async function exportFileMergePDF(gsaFiles: GSAFileData[]) {
  try {
    ...
    // Draw data curves for all files - exactly as in image
    gsaFiles.forEach((file, fileIndex) => {
      const validData = file.sieveData
      ...
      if (validData.length > 1) {
        ...
        // Draw data points with different symbols - exactly as in image
        validData.forEach((point) => {
          ...
          if (x >= chartX && x <= chartX + chartWidth) {
            // Draw different symbols for different files
            switch (file.symbol) {
              case "○":
                doc.circle(x, y, 1, "S")
                break
              case "□":
                ...
                break
              case "△":
                doc.triangle(x, y - 1, x - 1, y + 1, x + 1, y + 1, "S")
                break
              default:
                doc.circle(x, y, 1, "S")
            }
    ...
    doc.setFontSize(8)
    const legendX = chartX + chartWidth - 60
    const legendY = chartY + 15
    gsaFiles.forEach((file, index) => {
      doc.text(file.symbol, legendX, legendY + index * 6)
      doc.text(file.fileName, legendX + 5, legendY + index * 6)
    })
    ...
    // Classification data rows for all files
    gsaFiles.forEach((file, fileIndex) => {
      const results = file.analysisResults
      doc.rect(chartX, currentY, chartWidth, 8)
      doc.setFont("helvetica", "normal")
    ...
    // Save
    doc.save(`Grain_Size_Distribution_Comparison_${new Date().toISOString().split("T")[0]}.pdf`)
  } catch (error) {
